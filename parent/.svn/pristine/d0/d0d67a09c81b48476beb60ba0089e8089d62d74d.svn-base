...
function doProvAndCityRelation() {
    ...
    if ($("#chooseCity").length === 0) {
        city.append("<option id='chooseCity' value='0'>请选择城市</option>");
    }
    if ($("#chooseCounty").length === 0) {
        county.append("<option id='chooseCounty' value='0'>请选择城市</option>");
    }
    var sb = new StringBuffer();
    $.each(cityJson,
        function(i, val) {
...
function doCityAndCountyRelation() {
    ...
    var city_value = $("#citys").val();


    //获取园所
    ajaxGetUser(getAllSchoolByProvinceUrl,{province:province_value,city:city_value},function(ret,err){
        if(ret.success){
